function NAVIGASTRIC_group_preproc(subj_idx,smoothingSize)
...
subjects = cellstr(strcat('Subjects\','subj', sprintf('%.2d',subj_idx)));
...
% sessions = {'fMRI\RestingState','fMRI\CSV','fMRI\EBA','fMRI\MT'};
sessions = {'fMRI'};


params.anatdir = 'anatomy';
% params.anatwc  = '^s_S.*\.nii$';%% les images anatomiques originales commencent par anat et sont en .nii c'était avant maintenant (au CHSA sur images GE anonymisées, ça commence par sAW)
%params.anatwc  = '^.*\.nii$'; 
params.anatwc  = 's_S07_mprage_1iso_ipat2.nii';%% les images anatomiques originales commencent par anat et sont en .nii c'était avant maintenant (au CHSA sur images GE anonymisées, ça commence par sAW)

[params.funcdirs{1:length(subjects)}] = deal(sessions);
params.funcwc  = '^f450_.*\.nii$';%% les images fonctionelles originales commencent par un f !
%params.funcwc  = '^.*\.nii$';
