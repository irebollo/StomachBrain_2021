function [meany,meanY,sey,seY,firy,firY,firciy,firciY]=stanplot_group_b_params(Finterest,groupvar,timeaxis,baselinepoints,voxelcoord,params)
...
%%% get all arguments
if length(voxelcoord)==0  %%%% last argument is missing, find it in the SPM figure
    figure(1);
...
%getsubjectsdata;
% Replace old function getsubjectsdata.m
ngroups = params.ngroups;
groupn = params.groupn;
nsub = params.nsub;
totsub = params.totsub;
%for i=1:nsub
for i=1:totsub
    subjectsdir{i} = params.subjectsdir{i};
end
%feval(subjectmfile);
