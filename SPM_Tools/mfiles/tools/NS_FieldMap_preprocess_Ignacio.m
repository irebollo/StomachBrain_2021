function VDM=NS_FieldMap_preprocess_Ignacio(fm_dir,epi_dir,pm_defs,sessname)
%
...
% AA      epi_all = spm_select('List',epi_dir{sessnum},'^f.*\.img$');
...
elseif isstr(epi_dir)
   nsessions=1;
   sessnum=1;
% AA   epi_all = spm_select('List',epi_dir,'^f.*\.img$');
...
% AA  fm_imgs = spm_select('List',fm_dir,'^s.*\.img$');
   %fm_imgs = spm_select('List',fm_dir,'^*_flipped\.img$');   
   fm_imgs = spm_select('List',fm_dir,'^*\.img$'); % Changed by A. Moreno 2009-09-09 (images are not called *_flipped.img any more)
...
% AA      if nfiles~=3
% AA         msg=sprintf('Wrong number of field map (s*.img) images! There should be 3!');
% AA         error(msg);
% AA      else
         % Need first of two mag images and the phase
         % These may have been acquired in either order so need to check for this
% AA         nn=findstr(fm_imgs(1,:),'-');
% AA         if isempty(findstr(fm_imgs(2,:),fm_imgs(1,nn(1):nn(2))))
% AA            phase=fullfile(fm_dir,fm_imgs(1,:));
% AA            mag=fullfile(fm_dir,fm_imgs(2,:));
% AA         else
% AA            mag=fullfile(fm_dir,fm_imgs(1,:));
% AA            phase=fullfile(fm_dir,fm_imgs(3,:));
% AA         end
           mag=fullfile(fm_dir,fm_imgs(1,:));
           phase=fullfile(fm_dir,fm_imgs(2,:));         
