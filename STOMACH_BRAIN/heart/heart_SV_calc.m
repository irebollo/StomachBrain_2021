function HeartBeats = heart_SV_calc(cfg,data,HeartBeats,L)
...
% Hack Anne to take into account the reel L value (default was set at 30cm)


def             = [];
%def.ECG.channel = 'ECG1';
def.ECG.channel = 'BPCecg1';
def.ECG.scale   = [];
def.ECG.show    = 'yes';
...
def.QCmax       = .3;
def.RS1max      = .2;
def.LVETmax     = .4;  % mean = 0.25 ? 0.35 seconds in biopac doc
